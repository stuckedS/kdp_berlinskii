    ...
    <main>
        <article>
            <section>
                ...
                <form>
                    <div class="form-group">
                        ...
                        <a class='btn btn-primary' type='submit'
                            href=/adm_edit.php?table=serv&Idd='<?php echo $_GET["ID"] ?>'>Поиск</a>
                    </div>
                </form>
                <?php
                /* Все варианты сортировки */
                $sort_list = array(
                    'idFlights_asc' => '`idEmployees`',
                    'idFlights_desc' => '`idEmployees` DESC',
                    'Data_asc' => '`Surname`',
                    'Data_desc' => '`Surname` DESC',
                    'Airport_out_asc' => '`Name`',
                    'Airport_out_desc' => '`Name` DESC',
                    'Airport_in_asc' => '`Patronymic`',
                    'Airport_in_desc' => '`Patronymic` DESC',
                );
                ...
                $sth = $dbh->prepare("SELECT * FROM `Employees` ORDER BY {$sort_sql}");
                $sth->execute();
                ...
                <table table border=1px class='table_dark'>
                    <thead>
                        <tr>
                            <th>
                                <?php echo sort_link_th('Id Работника', 'idFlights_asc', 'idFlights_desc'); ?>
                            </th>
                            <th>
                                <?php echo sort_link_th('Фамилия', 'Data_asc', 'Data_desc'); ?>
                            </th>
                            <th>
                                <?php echo sort_link_th('Имя', 'Airport_out_asc', 'Airport_out_desc'); ?>
                            </th>
                            <th>
                                <?php echo sort_link_th('Отчество', 'Airport_in_asc', 'Airport_in_desc'); ?>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
